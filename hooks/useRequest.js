...
export default function useRequest() {
  ...
  const httpRequest = async (
    ...
    //   if (url) console.log("REQ RESPONSE", options);
      ...
      // A caller aborted via `signal`. Rethrow before the retry/status handling
      // below: a cancelled request has no `error.response`, so it would
      // otherwise fall through the whole block and resolve to `undefined`,
      // making an abort indistinguishable from an empty response. It must also
      // never be retried.
      if (isCancel(error)) {
        throw error;
      }
